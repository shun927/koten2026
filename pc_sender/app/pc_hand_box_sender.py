...
起動例（PowerShell / リポジトリルートで実行。venv を activate 済みの前提）：
  python .\pc_sender\app\pc_hand_box_sender.py `
  --source realsense --rs-fps 30 `
  --config .\pc_sender\config\endpoint.json `
...
入れ替え例（このプロジェクトの venv を使う想定。venv を activate 済み）：
...
def main() -> int:
    ...
    # ArUco lock state
    aruco_locked: bool = False          # True once plane is frozen
    locked_h: np.ndarray | None = None  # the frozen homography
    aruco_ok_since_ms: int | None = None  # when continuous detection started
    ...
    try:
        while True:
            ...
            if args.aruco_lock_after_ms > 0:
                if aruco_locked:
                    # Plane is frozen: always use it, never stale
                    ...
                    if aruco_ok:
                        ...
                        aruco_ok_since_ms = None  # reset on any miss
            # --- end lock ---
