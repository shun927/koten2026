...
このフォルダは、PCのWebカメラ映像から MediaPipe Hand Landmarker で手を推定し、箱の疑似3D（x,yは箱平面0..1、zは単眼の疑似深度）として両手21点をUDP(JSON)でTouchDesignerへ送信します。

## 作品システム（案）
- 送信PC（台下）：Webカメラ入力＋手推論（箱疑似3D: 両手21点）を生成してTouchDesigner PCへ送信
