...
このドキュメントは、TouchDesigner を「統合ハブ」として使い、画像処理PCから受け取った指先座標を作品用の box 座標に変換して Unity / 音PC へ配信するための要件・実装仕様です。

## 0. 運用構成（3台）
本番運用は、以下の3台構成を前提とします。
- 画像処理PC: カメラ入力、手検出、ArUco推定、TouchDesigner への座標送信を担当
- TD / Unity PC: TouchDesigner を統合ハブとして動かし、受信座標の整形と Unity 連携を担当
- 音PC: sound アプリ / パッチで OSC を受信し、音響制御を担当

補足:
- TouchDesigner と Unity は同一PCで動かす
- 音PCは TD / Unity PC とは別PCで動かす
- 画像処理PC は TouchDesigner に対して UDP / JSON を送信し、TD / Unity PC は音PCへ OSC / UDP を送信する

## 1. 役割
- 入力: 画像処理PC → touch（UDP / JSON）
- 処理: 左右割り当て、平滑化、範囲制限、ロスト処理、box座標化（0..1）
- 出力1: touch → Unity（同一PC連携）
- 出力2: touch → 音PC（OSC / UDP）
...
## 7. OSC出力（固定）
TD / Unity PC と音PCが分離された3台運用でも扱いやすいように、左右2chを固定で送る前提。

### 7.1 推奨（2点版：手首 + 人差し指先）
左右の手について、2点ぶんの座標を固定で送る。
- 左手首: `/box/hand/left/wrist` に `x y z valid`（float,float,float,int）
- 左人差し指先: `/box/hand/left/index_tip` に `x y z valid`（float,float,float,int）
- 右手首: `/box/hand/right/wrist` に `x y z valid`（float,float,float,int）
- 右人差し指先: `/box/hand/right/index_tip` に `x y z valid`（float,float,float,int）
- `x,y` は箱平面 `0..1`、`z` は疑似深度 `z_like`

### 7.2 互換（指先だけ）
旧仕様と同じ形が必要な場合だけ、疑似3Dでは `z=z_like` を送る。
...
### 7.3 OSC Out の宛先IP
- **Unity（同一PC）**: `127.0.0.1` を宛先にする
- **音PC（Thunderbolt別PC）**: 音PCの Thunderbolt 側IPを宛先にする
  - 例：音PC が `192.168.20.2` の場合、TouchDesigner の `osc_out_sound` は `192.168.20.2` 宛て
  - 音PC 側のファイアウォールで OSC 受信ポートを許可すること（`docs/requirements_network_pc_direct.md` §4.3 参照）
...
4. left/right 用 CHOP へ値を反映（`wrist xyz`, `index_tip xyz`, `valid`）

### 座標処理（left/right 共通）
1. `*_in`（CHOP: `wrist xyz`, `index_tip xyz`, `valid`）
